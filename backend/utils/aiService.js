...
export const chatCompletion = async ({ system, user, temperature = 0.7 }) => {
  ...
  if (!c) {
    return {
      ok: false,
      content:
        "AI features are disabled. Set GEMINI_API_KEY in the backend .env to enable real AI responses. Meanwhile, here is a fallback response.",
    };
  }
...
export const SYSTEM_PROMPTS = {
  ...
  suggestion:
    "You are a helpful habit coach. Based on the user's goals, productive time, and past struggles, suggest exactly 3 personalised habits. Return valid JSON only with this shape: {\"suggestions\":[{\"name\":\"...\",\"description\":\"...\",\"frequency\":\"Daily|Weekly|Monthly\",\"category\":\"Health|Fitness|Learning|Mindfulness|Productivity|Social|Finance|Creative|Other\",\"icon\":\"<emoji>\",\"reason\":\"...\"}]}. No prose outside JSON.",
