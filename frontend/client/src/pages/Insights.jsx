...
export default function Insights() {
        ...
        if (cached) {
          try {
            ...
            if (parsed.generatedAt) {
              setReportGeneratedAt(new Date(parsed.generatedAt));
            }
          } catch {
  ...
  const thisWeekKeys = useMemo(
    () => new Set(thisWeek.map((day) => day.key)),
    [thisWeek]
  );

  const thisWeekLogs = useMemo(
    () => logs.filter((log) => thisWeekKeys.has(log.completedDate)),
    [logs, thisWeekKeys]
  );
  ...
  const dailyData = thisWeek.map((day) => {
    const count = thisWeekLogs.filter(
      (log) => log.completedDate === day.key
    ).length;
  ...
  const compareData = thisWeek.map((day, index) => {
    const thisCount = thisWeekLogs.filter(
      (log) => log.completedDate === day.key
    ).length;

    const lastCount = lastWeekLogs.filter(
      (log) => log.completedDate === lastWeek[index].key
    ).length;
  ...
  const categoryData = useMemo(() => {
    ...
    for (const habit of habits) {
      habitCategoryMap[habit._id] = habit.category;
    }

    const counts = {};

    for (const log of thisWeekLogs) {
      const category = habitCategoryMap[log.habitId];
  ...
  const streakBoard = useMemo(() => {
    ...
    for (const habit of habits) {
      ...
      out[habit._id] = streakFromKeys(keys);
    }
        ...
        <div className="card p-5">
          ...
          <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 gap-2">
            {habits
              .filter((habit) => !habit.isArchived)
              .map((habit) => {
                const streak = streakBoard[habit._id];
                    ...
                    <div className="min-w-0 flex-1">
                      ...
                      <div
                        className={`text-xs font-medium ${
                          current > 0 ? "text-orange-500" : "text-faint"
                        }`}
                      >
