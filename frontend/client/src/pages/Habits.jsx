...
export default function Habits() {
  ...
  const load = async () => {
    ...
    try {
      ...
      for (const log of rangeRes.data) {
  ...
  const activeCount = habits.filter((habit) => !habit.isArchived).length;
  const archivedCount = habits.filter((habit) => habit.isArchived).length;
  ...
  const save = async (data) => {
    ...
    try {
      ...
      if (editing) {
        ...
        setLogsByHabit((prev) => ({ ...prev, [res.data._id]: [] }));
      }
    ...
    <div className="space-y-6 animate-fade-in">
      ...
      {filtered.length === 0 ? (
        ...
        <div className="space-y-2">
          {filtered.map((habit) => {
            const keys = logsByHabit[habit._id] || [];
